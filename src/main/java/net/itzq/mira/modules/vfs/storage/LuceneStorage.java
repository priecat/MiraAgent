...
public class LuceneStorage implements AutoCloseable {
    ...
    public LuceneStorage(String indexPath) {
        ...
        log.debug("LuceneStorage 已创建（延迟初始化）: indexPath={}", indexPath);
    }

    /**
     * 确保索引已打开（幂等、线程安全）。若打开失败则保持未初始化，全文检索降级为不可用。
     *
     * @param createIfMissing 磁盘上不存在索引目录时是否创建。
     *                        <ul>
     *                          <li>写入路径（indexDocument）传 true：首次索引时创建目录</li>
     *                          <li>读取/删除路径（search、deleteDocument、deleteByPrefix、clearIndex）
     *                              传 false：仅打开已存在的索引，不产生空目录，
     *                              保持与 VFS 读操作不创建文件的懒加载语义一致</li>
     *                        </ul>
     */
    private synchronized void ensureInitialized(boolean createIfMissing) {
        if (initialized) {
            return;
        }
        if (!createIfMissing && !new java.io.File(indexPath).exists()) {
            return;
        }
        try {
            initIndex();
