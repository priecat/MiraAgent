...
public class VkbFileEditTool {
    ...
    @Tool(name = ToolFun.TOOL_VKB_FILE_EDIT,
          description = "在“工作空间”中对文件执行精确字符串替换。\n\n" +
                  "使用说明：\n"
    ...
    public String fileEdit(@ToolParam(description = "要编辑的文件绝对路径（必填）") String filePath,
        ...
        try (VKB kb = VKB.load(sessionId)) {
            ...
            if (replaceAllFlag) {
                ...
                newContent = fileContent.replaceFirst(Pattern.quote(oldString), Matcher.quoteReplacement(newString));
            }
            ...
            // 写回文件
            kb.write(path.toString(), newContent);

            return String.format("文件编辑成功: %s\n%d 处匹配已替换", filePath, replaceAllFlag ? matchCount : 1);
        } catch (Exception e) {
