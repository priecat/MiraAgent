 ...
 *
 * 主代理通过此工具委派代码探索任务给vfs-explorer子代理。
 * 子代理拥有独立上下文
 ...
 */
@Slf4j
public class VfsExplorerTool {

    /** code-explorer 系统提示词模板路径 */
    private static final String PROMPT_PATH = "assets/prompt/vfs-explorer.md";

    @Tool(name = ToolFun.TOOL_VFS_Explorer,
          display = "探索工作空间",
          subAgent = true,
          description = "启动代码探索子代理，在工作空间中进行广泛的搜索和探索。\n\n"
                  + "当任务需要广泛的工作空间探索而非读取几个特定文件时使用此工具。\n"
                  + "子代理捆绑了 vfs_list_files、vfs_glob、vfs_grep、vfs_file_read 等工具，使大规模搜索更高效。\n\n"
                  + "使用场景：\n"
                  + "- 需要理解工作空间或文件夹的结构\n"
                  + "- 识别模块、包或子项目\n"
                  + "- 查找某个功能、概念或行为的实现位置\n"
    ...
    public String codeExplorer(
        ...
        try {
            log.info("启动 vfs-explorer 子代理，任务: {}", query.length() > 100 ? query.substring(0, 100) + "..." : query);

            String prompt = PromptLoader.prompt(PROMPT_PATH);

            // 创建子代理
            SubAgent explorer = new SubAgent(contextHolder, "explorer", prompt);
